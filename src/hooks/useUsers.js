  ...
  export function useUsers() {
      ...
      try {
        return await dataSource(
          async () => {
            ...
            addNotification({ type: "success", title: "User added", message: `Added user ${userData.name}` });
            return newUser;
          },
          async () => {
            const body = {
              ...
              role: ROLE_TO_INT[userData.role] ?? 1,
              tenantId: 0,
        ...
        return await dataSource(
          async () => {
            ...
            if (idx !== -1) {
              ...
              addNotification({ type: "info", title: "User updated", message: `Updated user ${MOCK_USERS[idx].name}` });
            }
          ...
          async () => {
            ...
            addNotification({ type: "info", title: "User updated", message: `Updated user ${userData.name}` });
            return data;
          }
        );
      } finally {
      ...
      try {
        await dataSource(
          async () => {
            ...
            addNotification({ type: "error", title: "User(s) deleted", message: `Deleted: ${names}` });
          },
          async () => {
            if (ids.length === 1) {
            ...
            addNotification({ type: "error", title: "User(s) deleted", message: `Deleted ${ids.length} user(s)` });
          }
        );
      } finally {
        setLoading(false);
      ...
      try {
        await dataSource(
          async () => {
            ...
            addNotification({ type: "warning", title: "User(s) deactivated", message: `Deactivated ${ids.length} user(s)` });
          },
          async () => {
            if (ids.length === 1) {
      ...
      try {
        await dataSource(
          async () => {
            ...
            addNotification({ type: "success", title: "User(s) activated", message: `Activated ${ids.length} user(s)` });
          },
          async () => {
            if (ids.length === 1) {
    ...
    return {
      ...
      fetchUsers, getUser, addUser, updateUser,
      deleteUser, deleteUsers, deactivateUsers, activateUsers,
    };
  }